...
fn lists(
    ...
    list_char: []const u8,
    string: []const u8,
    list_string4: [][]const u8,
    list_list_string2: [][][]const u8,
    matrix: [][]i32,
) !void {
    // Aren't these lists beautifull?
    _ = .{ n, list_int, size, list_char, string, list_string4, list_list_string2, matrix };
}
...
pub fn main() !void {
    ...
    const list_int = try lineReader.readNElements(i32, ' ', @intCast(n));
    const size = try lineReader.readValue(i32);
    const list_char = try lineReader.readType([]const u8, &[_]usize{@intCast(size)}, ' ');
    const string = try lineReader.read();
    const list_string4 = try lineReader.readType([][]const u8, &[_]usize{@intCast(size)}, ' ');
    const list_list_string2 = try lineReader.readType([][][]const u8, &[_]usize{ 2, 2 }, ' ');
    const matrix = try lineReader.readType([][]i32, &[_]usize{ @intCast(size), @intCast(size) }, ' ');
