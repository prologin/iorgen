...
class ParserKotlin:
    ...
    def read_line(
        ...
        if type_.main == TypeEnum.STRUCT:
            ...
            lines = [indent + f'{f"var {name}: {type_str(type_)}" if decl else words} = reader.readLine().split(" ").let {{']
            lines.append(
                indent + INDENTATION
                + "{}(".format(
        ...
        if type_.main in (TypeEnum.INT, TypeEnum.FLOAT, TypeEnum.CHAR, TypeEnum.STR):
            ...
            if type_.encapsulated.main == TypeEnum.CHAR:
                ...
                command = (
                   'reader.readLine().split(" ").map(String::{})'.format(
    ...
    def call(self, reprint: bool) -> List[str]:
        ...
        lines.append(" */")
        lines.append(
            f"fun {var_name(self.input.name)}("
        )
        ...
        if reprint:
            for variables in self.input.get_all_vars():
                if len(variables) == 1:
                    ...
                    lines.append(
                        INDENTATION + 'println("{}")'.format(f"${ {var_name(i.name)} } " for i in variables)
                    )
        else:
            lines.extend(
                [
    ...
    def print_line(self, name: str, type_: Type) -> str:
        ...
        if type_.main == TypeEnum.LIST:
            ...
            assert type_.encapsulated.main in (TypeEnum.INT, TypeEnum.FLOAT)
            return (
                f'println({name}.joinToString(" "))'
            )
        assert type_.main == TypeEnum.STRUCT
        fields = self.input.get_struct(type_.struct_name).fields
        return (
            f'println(' + ' '.join(f'${{ {name}.{var_name(f.name)} }}' for f in fields) + ')'
        )
