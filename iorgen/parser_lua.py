...
def read_line(name: str, type_: Type, input_data: Input,
    ...
    if type_.main == TypeEnum.LIST:
        ...
        if type_.encapsulated.main == TypeEnum.CHAR:
            return [
                '{} = {{}}'.format(name),
                'io.read():gsub(".",function({0}) table.insert({1}, {0}) end)'.
                format(inner, name)
            ]
    ...
    if type_.main == TypeEnum.STRUCT:
        ...
        iterator.pop_it()
        pattern = " ".join("(-?%d+)" if i.type.main == TypeEnum.INT else "(%S)"
        ...
        return [
            'local {} = {{string.match(io.read(), "{}")}}'.format(
                words, pattern), "{} = {{{}}}".format(
                    name,
                    ", ".join(i + " = " + j for (i, j) in zip(keys, values)))
        ]
    return [
        name + " = " + {
...
def read_lines(name: str, type_: Type, size: str, input_data: Input,
    ...
    if type_.main == TypeEnum.LIST:
        assert type_.encapsulated is not None
        inner = iterator.new_it()
        lines = read_lines("{}[{}]".format(name, inner), type_.encapsulated,
                           var_name(type_.encapsulated.size), input_data,
                           iterator)
        iterator.pop_it()
        return [
            '{} = {{}}'.format(name), 'for {} = 1, {} do'.format(inner, size)
        ] + [INDENTATION + i for i in lines] + ['end']
    assert type_.main == TypeEnum.STRUCT
    struct = input_data.get_struct(type_.struct_name)
    lines = [name + " = {}"]
    for i, field in enumerate(struct.fields):
        ...
        lines.extend(
            read_lines('{}["{}"]'.format(name, field.name), field.type, f_size,
                       input_data, iterator))
    return lines
...
def print_lines(name: str, type_: Type, input_data: Input,
    ...
    if type_.main == TypeEnum.LIST:
        ...
        return [indent + "for _, {} in ipairs({}) do".format(inner, name)
                ] + print_lines(inner, type_.encapsulated, input_data,
                                indent_lvl + 1) + [indent + "end"]
    assert type_.main == TypeEnum.STRUCT
    lines = []
    for i in input_data.get_struct(type_.struct_name).fields:
        lines.extend(
            print_lines('{}["{}"]'.format(name, i.name), i.type, input_data,
                        indent_lvl))
    return lines
